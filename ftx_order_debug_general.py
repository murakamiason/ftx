...
# decide upper and lower limit_boundary
# this will be decided automatically from historical data in the future
# like to optimize this boundary
...
# minimum_price_stride_order = -3
bea = minimum_order_amount*1
sea = minimum_order_amount*1

# entry_detect_time = 5
# exit_detect_time = 300
...
while True:
    try:
        # get total asset balance (usd balance)
...
while True:
    ...
    if score < minimum_score and entried == False:
        ...
        time.sleep(15)
        continue
    # first, you get traded future position, your usd balance, best bid and best ask
    while True:
        print("initial phase")
        index_unch = True
        # to avoid api limit breach
        time.sleep(0.1)
        try:
            # get traded future position(this only fetches future position but not spot balance)
            # somehow realizedPnl is refreshed whenever mark price changes, you gotta adjust it when you get your entry price
            ...
            for b in balances_list:
                if b["coin"] == accounting_currency:
                    ...
                    asset = float(b["total"])
                    break
            # get order book to collect index
    ...
    if initial_position_size < minimum_order_amount:
        ...
        # take a rest of 1 second and check if orders are filled
        time.sleep(1)
        while True:
            try:
        ...
        if initial_position_size < position_size:
            ...
            # this means your entry orders were posted and not filled or your entry orders were not posted because of postOnly orders or both of your buy entry order and sell entry order were filled and still position is less than initial position(very rare case assumed)
            print("no entry")
            while True:
                while True:
                    try:
                ...
                if len(open_orders) == 0:
                    # no need to cancel orders because you know there is no open positions
                    ...
                    if wait_forever == True:
                        ...
                        for o in open_orders:
                            # you cancel and reorder if buy or sell best price is changed(think that it is better that you have judgement on each of them)
                            if o["side"] == "buy":
                        ...
                        # you check here if you exit in the mid of your entry detect time
        ...
        if initial_position_side == "buy":
            # you gotta adjust because average entry price could be off minimum price stride
            # initial_position_entry_price = ceil_floor_general(price = initial_position_entry_price, minimum_price_stride_order = minimum_price_stride_order)[1]
            while True:
                try:
            ...
            # initial_position_entry_price = ceil_floor_general(price = initial_position_entry_price, minimum_price_stride_order = minimum_price_stride_order)[0]
            while True:
                try:
            ...
            while True:
                try:
                    ...
                    print(e)
                    time.sleep(1)
        # wait and check status of your order
        # if you do not sleep well, you cannot get correct positions because of latency
        time.sleep(1)
        while True:
            try:
                positions = ftx.private_get_positions()["result"]
        ...
        if position_size < minimum_order_amount:
            ...
            # this means your exit orders were posted and not filled or your exit orders were not posted because of postOnly orders or partially filled
