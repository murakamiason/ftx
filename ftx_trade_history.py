...
print("inititate fetching trade history of {0} in {1} from {2} JST".format(symbol, exchange_name, end_datetime))

hist_trades = []
current_date  = pd.DataFrame(ftx.public_get_markets_market_name_trades(params = {"market_name":symbol, "limit":max_size, "end_time":end_time})["result"])["time"].apply(lambda x : x[:10])[0]

ts = time.time()
while end_time < last_end_time:
    while True:
        try:
            trades = ftx.public_get_markets_market_name_trades(params = {"market_name":symbol, "limit":max_size, "end_time":end_time})["result"]
            break
    ...
    if current_date  != latest_date:
        ...
        df_old.to_csv("executions_{0}_{1}_{2}.csv".format(symbol.replace("/", "-"), exchange_name, current_date))
        ...
        print("saved executions {0} {1} {2}".format(symbol.replace("/", "-"), exchange_name, current_date))
